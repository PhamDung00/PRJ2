...
/**
 * ApexCharts Config Class for extending users options with pre-defined ApexCharts config.
 *
 * @module Config
 **/
export default class Config {
  ...
  init({ responsiveOverride }) {
    ...
    if (opts && typeof opts === 'object') {
      ...
      // If users has specified a dark theme, make the tooltip dark too
    ...
    // default-config < global-apex-variable-config < users-defined-config

    // get GLOBALLY defined options and merge with the default config
    let mergedWithDefaultConfig = Utils.extend(newDefaults, window.Apex)

    // get the merged config and extend with users defined config
  ...
  extendYAxis(opts, w) {
    ...
    // users can provide either an array or object in yaxis config
    if (opts.yaxis.constructor !== Array) {
      // convert the yaxis to array if users supplied object
